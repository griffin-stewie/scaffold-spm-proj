...
final class ManifestRewriterTests: XCTestCase {
    ...
    override func setUp() {
        do {
            try resetPackageSwift(rootDirectoryPath: rootDirectoryPath, manifestPath: manifestPath, manifestContents: inputManifestContent)
        } catch {
...
extension ManifestRewriterTests {
    private func resetPackageSwift(rootDirectoryPath: AbsolutePath, manifestPath: AbsolutePath, manifestContents: String) throws {
        fileSystem = InMemoryFileSystem()
        try fileSystem.createDirectory(rootDirectoryPath, recursive: true)
        try fileSystem.writeFileContents(manifestPath, bytes: ByteString(Array(manifestContents.utf8)))
    }
}
